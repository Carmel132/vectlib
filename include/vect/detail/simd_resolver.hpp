...
template <typename MaskE, typename TrueE, typename FalseE> class WhereOp;
} // namespace vect::expr
...
template <typename T> struct SimdResolver {
  using valueType = T;
...
template <typename T, size_t N> struct SimdResolver<core::Vector<T, N>> {
  using valueType = T;
  static constexpr size_t dim = N;
  static constexpr bool isAligned = true;
};

template <typename T, size_t N> struct SimdResolver<core::VecScalar<T, N>> {
  using valueType = T;
  static constexpr size_t dim = 0;
  static constexpr bool isAligned = true;
};

template <typename E, typename Op> struct SimdResolver<expr::UnaryOp<E, Op>> {
...
struct SimdResolver<expr::BinaryOp<L, R, Op>> {
  ...
  static constexpr bool isAligned =
      SimdResolver<L>::isAligned && SimdResolver<R>::isAligned;
};
...
struct SimdResolver<expr::WhereOp<M, T, F>> {
  ...
  static constexpr bool isAligned = SimdResolver<M>::isAligned &&
                                    SimdResolver<T>::isAligned &&
                                    SimdResolver<F>::isAligned;
};
} // namespace vect::detail